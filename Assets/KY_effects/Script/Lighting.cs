...
void Update (){
	...
	IEnumerator setRev (){
		if( revOnTime > 0){
			yield return new WaitForSecondsRealtime( revOnTime );
			lighting *= -1; 
		}
	}
	...
	IEnumerator keepOn (){
		if(  keepOnTime > 0){
			yield return new WaitForSecondsRealtime( keepOnTime );
			lightKeepFlg = true;
		}
	}
	...
	IEnumerator setFlashingOff (){
		if(  flashingOff > 0){
			yield return new WaitForSecondsRealtime( flashingOff );
			flashingFlg = false;
